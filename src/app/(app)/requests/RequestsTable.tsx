...
export default function RequestsTable({ rows }: { rows: RequestRow[] }) {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white shadow-sm">
      ...
      {shown.length === 0 ? (
        <p className="px-5 py-10 text-center text-sm text-zinc-400">No requests in this view.</p>
      ) : (
              ...
              {shown.map((r) => (
                <tr key={r.id} className="transition-colors hover:bg-brand-50/40">
                  <td className="px-5 py-3">
                    <Link href={`/requests/${r.id}`} className="block font-medium text-zinc-900 hover:text-brand-700">
                      {r.description}
                    </Link>
                  ...
                  <td className="px-4 py-3 text-zinc-700">{r.requesterName}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <StageBar filled={r.stageFilled} />
                      <span className="text-[13px] capitalize text-zinc-500">{r.status.replace(/_/g, " ")}</span>
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-right text-[17px] tabular-nums text-zinc-900">{r.valueLabel}</td>
                  <td className={`whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums ${r.ageAmber ? "font-medium text-amber-700" : "text-zinc-500"}`}>
                    {r.ageDays}d
                  </td>
                </tr>
              ))}
