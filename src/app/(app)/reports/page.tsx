...
export default async function ReportsPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-[38px] font-semibold leading-none tracking-tight text-zinc-900">Reports</h1>
        <p className="mt-2 text-sm text-zinc-500">Total spend to date: {formatNaira(totalSpend)} (across all fully-created purchase orders)</p>
      </div>
              ...
              <tbody className="divide-y divide-zinc-100">
                {pendingApprovals.map((a, i) => (
                  <tr key={i} className="transition-colors hover:bg-brand-50/40">
                    <td className="px-5 py-2.5 font-medium text-zinc-900">{a.request_number}</td>
                    <td className="max-w-xs truncate px-4 py-2.5 text-zinc-700">{a.description}</td>
                    <td className="px-4 py-2.5 text-zinc-700">{a.step_order}</td>
                    <td className="px-4 py-2.5 capitalize text-zinc-700">{a.approver_role.replace(/_/g, " ")}</td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-right text-[17px] tabular-nums text-zinc-900">{formatNaira(a.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full min-w-[480px] text-sm">
              ...
              <tbody className="divide-y divide-zinc-100">
                {expiringCerts.map((c, i) => (
                  <tr key={i} className="transition-colors hover:bg-brand-50/40">
                    <td className="px-5 py-2.5 font-medium text-zinc-900">{c.vendor_name}</td>
                    <td className="px-4 py-2.5 text-zinc-700">{c.label}</td>
                    <td className="px-4 py-2.5 text-zinc-700">{c.expiry_date}</td>
