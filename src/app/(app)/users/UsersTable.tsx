      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[600px] text-sm">
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {shown.map((u) => (
              <tr key={u.id} className="transition-colors hover:bg-brand-50/40 dark:hover:bg-zinc-800">
                <td className="px-5 py-3 font-medium text-zinc-900 dark:text-zinc-100">{u.fullName}</td>
                <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300">{u.email}</td>
                <td className="px-4 py-3 capitalize text-zinc-700 dark:text-zinc-300">{u.role.replace(/_/g, " ")}</td>
                <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300">{u.departmentName}</td>
                <td className="px-4 py-3">
                  {u.isActive ? (
                    <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-500/10 dark:text-green-400">Active</span>
                  ) : (
                    <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">Deactivated</span>
                  )}
                </td>
                {rigsourceEnabled && (
                  <td className="px-4 py-3">
                    {u.rigsourceInvitedAt ? (
                      <span className="text-xs text-zinc-500 dark:text-zinc-400">Invited {new Date(u.rigsourceInvitedAt).toLocaleDateString()}</span>
                    ) : (
                      <form action={grantRigSourceAccess}>
                        <input type="hidden" name="user_id" value={u.id} />
                        <button className="text-xs font-medium text-brand-600 hover:underline dark:text-brand-400">Grant access</button>
                      </form>
                    )}
                  </td>
                )}
