...
export default function PurchaseOrdersTable({ rows }: { rows: PoRow[] }) {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {shown.length === 0 ? (
        <p className="px-5 py-10 text-center text-sm text-zinc-400 dark:text-zinc-500">No purchase orders in this view.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[680px] text-sm">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {shown.map((po) => (
                <tr key={po.id} className="transition-colors hover:bg-brand-50/40 dark:hover:bg-zinc-800/60">
                  <td className="px-5 py-3">
                    <Link href={`/purchase-orders/${po.id}`} className="block font-medium text-zinc-900 hover:text-brand-700 dark:text-zinc-100 dark:hover:text-brand-400">
                      {po.title}
                    </Link>
                    <div className="mt-0.5 flex items-center gap-1.5 text-[13px] text-zinc-400 dark:text-zinc-500">
                      {po.poNumber}
                      {po.fxFlagged && <span className="rounded-full bg-amber-50 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">FX moved</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300">{po.vendorName}</td>
                  <td className="px-4 py-3">
                    <StatusBadge status={po.status} />
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-right text-[17px] tabular-nums text-zinc-900 dark:text-zinc-100">{po.landedCostLabel}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-zinc-500 dark:text-zinc-400">{po.ageDays}d</td>
                </tr>
              ))}
            </tbody>
          </table>
