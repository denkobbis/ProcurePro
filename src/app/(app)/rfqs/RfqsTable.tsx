...
export default function RfqsTable({ rows }: { rows: RfqRow[] }) {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white shadow-sm">
      ...
      {shown.length === 0 ? (
        <p className="px-5 py-10 text-center text-sm text-zinc-400">No RFQs in this view.</p>
      ) : (
              ...
              {shown.map((rfq) => (
                <tr key={rfq.id} className="transition-colors hover:bg-brand-50/40">
                  <td className="px-5 py-3">
                    <Link href={`/rfqs/${rfq.id}`} className="block font-medium text-zinc-900 hover:text-brand-700">
                      {rfq.requestDescription}
                    </Link>
                    <div className="mt-0.5 text-[13px] text-zinc-400">{rfq.requestNumber}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-zinc-700">{rfq.leadTimeLabel}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-zinc-500">{rfq.raisedDaysAgo}d ago</td>
                </tr>
              ))}
            </tbody>
          </table>
