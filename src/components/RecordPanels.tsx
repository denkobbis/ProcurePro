    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {subline && <p className="mt-1 text-sm text-zinc-500">{subline}</p>}
        </div>
...
export function Stepper({ steps }: { steps: StepDef[] }) {
  return (
    <div className="grid gap-2 rounded-lg border border-zinc-200 bg-white p-4 print:hidden" style={{ gridTemplateColumns: `repeat(${steps.length}, minmax(0, 1fr))` }}>
      {steps.map((s, i) => (
        <div key={i} className="flex flex-col items-start gap-1.5">
          <div className="flex w-full items-center">
            <span
              className={`h-[11px] w-[11px] shrink-0 rounded-full border-2 ${
                s.state === "done" ? "border-brand-600 bg-brand-600" : s.state === "current" ? "border-brand-600 bg-white ring-3 ring-brand-100" : "border-zinc-300 bg-white"
              }`}
            />
            {i < steps.length - 1 && <span className={`h-px flex-1 ${s.state === "done" ? "bg-brand-600" : "bg-zinc-200"}`} />}
          </div>
          <span className={`text-[12px] font-medium ${s.state === "todo" ? "text-zinc-400" : "text-zinc-900"}`}>{s.label}</span>
          {s.date && <span className="text-[11px] text-zinc-400">{s.date}</span>}
        </div>
      ))}
    </div>
  );
...
export function FactsPanel({ facts }: { facts: Array<{ label: string; value: ReactNode; full?: boolean }> }) {
  return (
    <dl className="grid grid-cols-1 gap-x-6 sm:grid-cols-2">
      {facts.map((f, i) => (
        <div key={i} className={`flex items-baseline justify-between gap-3 border-b border-zinc-100 py-2.5 first:pt-0 ${f.full ? "sm:col-span-2" : ""}`}>
          <dt className="shrink-0 text-[13px] text-zinc-500">{f.label}</dt>
          <dd className="text-right text-sm tabular-nums text-zinc-900">{f.value}</dd>
        </div>
      ))}
    </dl>
  );
...
export function NotePanel({ title, tone = "default", children }: { title?: string; tone?: "default" | "warn"; children: ReactNode }) {
  return (
    <div className={`rounded-md px-4 py-3 text-sm ${tone === "warn" ? "border-l-[3px] border-amber-500 bg-amber-50 text-amber-800" : "bg-zinc-50 text-zinc-700"}`}>
...
export function TimelineList({ items }: { items: Array<{ title: ReactNode; meta?: string; when?: string }> }) {
  if (items.length === 0) return <p className="text-sm text-zinc-400">Nothing yet.</p>;
  return (
      ...
      {items.map((it, i) => (
        <li key={i} className="flex gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-sm text-zinc-900">{it.title}</div>
            <div className="mt-0.5 flex gap-2 text-[12px] text-zinc-400">
    ...
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
      {stats.map((s, i) => (
        <div key={i} className="border-l-2 border-brand-600 pl-3">
          <div className="text-[28px] font-semibold leading-none tabular-nums text-zinc-900">{s.value}</div>
